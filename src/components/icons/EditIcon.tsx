import * as React from "react"
import { SVGProps } from "react"
export const EditIcon = (props: SVGProps<SVGSVGElement>) => {
    let { className, fill } = props
    if (!className) className = ""
    if (!fill) fill = "#1D3B6D"
return <svg width="24" height="24" viewBox="0 0 24 24" fill="none" {...props}  className={`${className} inline-block ml-1 mr-1`}>
<path d="M7 17.0134L11.413 16.9984L21.045 7.45839C21.423 7.08039 21.631 6.57839 21.631 6.04439C21.631 5.51039 21.423 5.00839 21.045 4.63039L19.459 3.04439C18.703 2.28839 17.384 2.29239 16.634 3.04139L7 12.5834V17.0134ZM18.045 4.45839L19.634 6.04139L18.037 7.62339L16.451 6.03839L18.045 4.45839ZM9 13.4174L15.03 7.44439L16.616 9.03039L10.587 15.0014L9 15.0064V13.4174Z" fill={fill}/>
<path d="M5 21H19C20.103 21 21 20.103 21 19V10.332L19 12.332V19H8.158C8.132 19 8.105 19.01 8.079 19.01C8.046 19.01 8.013 19.001 7.979 19H5V5H11.847L13.847 3H5C3.897 3 3 3.897 3 5V19C3 20.103 3.897 21 5 21Z" fill={fill}/>
</svg>

}